...
elasticsearch.prototype.version = function (prefix, callback) {
  ...
  self.baseRequest.get(self.base.host, function (err, response) {
    ...
    if (!self.searchBody.sort) {
      // if no sorting specified in search body, sort by @timestamp by default
      self.searchBody.sort = [{"@timestamp":{"order":"asc","unmapped_type":"boolean"}}];
    }
...
elasticsearch.prototype.getIndexForDate = function (indexPrefix, date, callback) {
  var self = this
  {
    var dateFrom = Date.UTC(date.getUTCFullYear(),date.getUTCMonth(),date.getUTCDate());
    var dateTo = Date.UTC(date.getUTCFullYear(),date.getUTCMonth(),date.getUTCDate(),23,59,59,999);

    var dateRangeQuerySpec = {
      "fields": ["@timestamp"],
      "index_constraints": {
        "@timestamp": {
          "max_value": {
            "gte": dateFrom, "format": "epoch_millis"
          },
          "min_value": {
            "lte": dateTo, "format": "epoch_millis"
          }
        }
      }
    }
    ...
    var esRequest = {
      'url': self.base.url + '/' + (indexPrefix != '*'? indexPrefix : '') + '*' + '/_field_stats?level=indices',
      'method': 'POST',
      'body': JSON.stringify(dateRangeQuerySpec),
      headers: {
        'kbn-version': (self.ESversion && self.ESversion < 5) ? '4.5.4' : '5.5.1'
      }
    }
    aws4signer(esRequest, self.parent)
...
elasticsearch.prototype.getData = function (limit, offset, callback) {
  ...
  if (self.lastScrollId !== null) {
    ...
    self.baseRequest.post(searchRequest, function requestResonse (err, response) {
      if (err) {
        callback(err, [])
        return
      } else if (response.statusCode !== 200) {
